...
describe('DBService', () => {
  ...
  describe('.connectDatabase', () => {
    ...
    it('should fail and throw exception when database URL is invalid', () => {
      const databaseURL = faker.string.sample()
  ...
  describe('.migrateDatabase', () => {
    ...
    it('should fail and throw exception when migration fails', () => {
      const migrationsFolder = faker.string.sample()
      vi.mock('drizzle-orm/postgres-js/migrator', (importOriginal) => {
        const original =
          importOriginal<typeof import('drizzle-orm/postgres-js/migrator')>()
        return {
          ...original,
          migrate: vi.fn(),
        }
      })
      vi.mocked(migrate).mockRejectedValue(new Error('failed'))
